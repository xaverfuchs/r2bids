...
#' valid participant IDs, and session description. The function can optionally also check for proper sex encoding ("m", "f", or "o"). It attempts to convert invalid data and
...
#' @examples
#' # Example dataset
#' example_data <- data.frame(ParticipantID = c(1, 2), Session = c(1, 1), sex = c("Male", "Female"), weird_variable=c("1_2", "3_4"))
#' check_input_data(data = example_data, participant_col = "ParticipantID", session_col = "Session", sex_col = "sex", ignore_cols=c("weird_variable"))
...
check_input_data <- function(data, participant_col = "participant_id", session_col = "session", sex_col = NULL, ignore_cols=NULL) {
  # Convert data to data frame
  data <- as.data.frame(data)

  # Convert all character variables to snake_case
  ...
  #ignore the ignore col and sex col
  if (!is.null(ignore_cols)) {
    character_cols <- character_cols[!character_cols %in% ignore_cols]
  }

  # if (!is.null(sex_col)) {
  #   character_cols <- character_cols[!character_cols %in% sex_col]
  # }
  ...
  # Check if participant and session variables are spelled correctly and numeric
  ...
  # if exists: rename sex variable
  if (!is.null(sex_col)) {
    if (!(sex_col %in% colnames(data))) {
      stop("Error: The specified sex column does not exist in the data.")
    }
  ...
  if (!is.null(sex_col)) {
    ...
    if (any(!data[["sex"]] %in% c("m", "f", "o"))) {
      warning(paste("Invalid values found in sex column. Please recode to 'm', 'f', or 'o'."))
    }
  }
  return(data)
}
