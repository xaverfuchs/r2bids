...
```{r}
B006Data_bids <- check_input_data(B006Data_bids, 
...
write_task_tsv(data = B006Data_bids, 
               bids_dir = output_dir, data_type = "beh", 
               filename_prefixes = c("sub-", "ses-", "task-", "run-"),
               filename_variables = c("participant_id", "session", "LID"="task", "run"),
               path_prefixes = c("sub-", "ses-"),
               path_variables = c("participant_id", "session"),
               ignore_variables = c("age", "sex", "arm_top", "length_lower_arm"))

```
Now we will write the participants.tsv

```{r}
write_participants_tsv(data = B006Data_bids, bids_dir = output_dir,
...
```{r}
write_metadata(bids_dir = output_dir, 
               task_name = "LID", 
               data_type = "beh",
               meta_data = meta_data)
```
...
```{r}
data_loaded <- read_bids(bids_dir = "example_bids2", 
                         data_type = "beh", 
                         filename_prefixes =  c("sub-", "ses-", 
                                                "task-", "run-"), 
                         filename_variables = c("participant_id", 
                                                "session", "task", "run"))
```
